	...
	function removeIf(candidateFunction,tree,funcName,param) {
		...
		for(var i in tree) { 
			if(tree[i]['type'] == 'IfStatement') {
				...
				if(tree[i]['consequent']['type'] == 'BlockStatement')
					var consequent = code.substring(tree[i]['consequent']['range'][0]+1, tree[i]['consequent']['range'][1]-1);
				else
					var consequent = code.substring(tree[i]['consequent']['range'][0], tree[i]['consequent']['range'][1]);
				...
				if(tree[i]['alternate'] != null) {
					if(tree[i]['alternate']['type'] == 'BlockStatement')
						alternate = code.substring(tree[i]['alternate']['range'][0]+1, tree[i]['alternate']['range'][1]-1);
					else
						alternate = code.substring(tree[i]['alternate']['range'][0], tree[i]['alternate']['range'][1]);
				}
				if(funcName == "function") {
...
//				console.log(code);
				...
				if(cursor > max) {
					...
					cursor -= len;
					reduced += len;	
				} else if(cursor > min && cursor < max){
					codeStr = codeStr.substring(0,min) + codeStr.substring(code['body']['range'][0]-reduced+1, code['body']['range'][1]-reduced-1) + codeStr.substring(max);
					break;
				}
			}
	...
	function getEvaluations(code,cursor,incompleteCode, strings) {
		...
		for(var i=0; i<evals.length; i++) {
			var DOMStrings = dom.getPartialDOM(evals,strings,i);
...
//			console.log(wordTags);
	...
	function getDompletions(code,cursor,incompleteCode,strings) {
