define(function(require, exports, module) {
	...
	function inGlobal(varName,funcName,vars) {
		...
		for(var variable in vars['globals']) {
			if(vars['globals'][variable]['varName'] == varName)
				ret = true;
		}
		return ret;
	}
	
	function getValueFromGlobal(variable,vars,scope) {
		
		if(vars['debug'])
			console.log("getting value from global scope " + variable);
		
		var ret = "";
		var ts = parseInt(scope.split(",")[0]);
		var globals = vars['globals'];
		for (var k in globals){
			if(parseInt(k.split(",")[0]) <= ts && globals[k]['varName'] == variable) {
				ret = globals[k]['value'];
			}
		}
		return ret;	
	...
	function getValueFromScope(variable,vars,scope) {
		...
		if(ret == '')
			ret = getValueFromGlobal(variable,vars,targetScope);
				...
				if(expression['type'] == 'AssignmentExpression') {
					if(expression['left'] != null) {
						...
						if(!inScope(varName, vars, prevScope, curScope) && !inGlobal(varName,funcName,vars)) {
							...
							vars['variables'][prevScope + curScope] = variable;
							vars['globals'][prevScope + curScope] = variable;
							curScope++;
